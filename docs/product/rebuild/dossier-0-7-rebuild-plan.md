...
- 0.7.1 prepared on `release/0.7.1` and held for Kyle: the reply block became Your reply, calm and compact, with every reply said in plain words by Go and the reader alike, and the showcase now leads with a brainstorm about a codebase while the winter crossing brainstorm became a test fixture.
- Next: cut 0.7.1 when Kyle says, move the new npm packages to trusted publishing, then Ledger.
