...
Dossier turns **one JSON model** into **one self-contained HTML file**: no server, no external requests, the model embedded as a JSON island so an agent reads one block instead of scraping. The reader gets a contents rail, collapsed items, a summary table, and decision controls that write a reply line. You get the reply back as decisions.
